...
# 3. Interpolar dados de renda média e população para os bairros
...
criar_popup_pop <- function(data) {
  popup_html <- sprintf(
    "<b>Bairro:</b> %s<br/>
     <b>População (Domicílios):</b> %s<br/>
     <b>Percentual da cidade:</b> %.2f%%",
...
mapa_populacao <- mapview(
  ...
  zcol = 'pop_ph', 
  layer.name = "População (Domicílios Particulares)",
  alpha.regions = 0.7,
  popup = popups_pop,
